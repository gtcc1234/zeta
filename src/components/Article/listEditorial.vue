  ...
  <v-container fluid>
    ...
    <v-layout row>
      <v-flex xs10 sm8 md7 offset-md1>
        ...
        <v-layout row wrap v-for="content in editors" :key="content.id" class="ml-1">
          ...
          <v-flex xs12>
              ...
              <v-flex>
                <v-btn flat v-on:click="removeEditorial(content.id)">
...
<script>
  ...
  export default {
    ...
    methods: {
      removeEditorial: function (article) {
        let editRef = firebase.database().ref('Editorials')
        editRef.child(article).remove()
      }
    }
  }
</script>
